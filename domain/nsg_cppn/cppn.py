...
class Cppn:
    def __init__(self, num_neurons=2, num_layers=2, sigma=2.0):
        ...
        self.num_weight_genes = (
            self.min_neurons * self.min_neurons * (self.num_layers + 1)
        )
        self.activations = np.random.randint(
            len(self.act_funcs), size=[self.num_neurons, self.num_layers + 1]
        )
        self.weights = np.random.normal(
            size=[self.min_neurons * self.min_neurons, self.num_layers + 1], scale=sigma
        )
    ...
    def set_genome(self, genome):
        self.activations = np.reshape(
            genome[0 : self.num_neurons * (self.num_layers + 1)],
            [self.num_neurons, self.num_layers + 1],
        )
        self.weights = np.reshape(
            genome[self.num_neurons * (self.num_layers + 1) :],
            [self.min_neurons * self.min_neurons, self.num_layers + 1],
        )
        ...
        for layer in range(1, self.weights.shape[1] + 1):
            ...
            for neuron_id in range(this_layer_num_neurons):
                ...
                for input_neuron_id in range(self.min_neurons):
                    weight = self.weights[
                        neuron_id * self.min_neurons + input_neuron_id, layer - 1
                    ]
