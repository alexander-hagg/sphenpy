...
def get(population, domain):
    ...
    for i in range(len(phenotypes)):
        ...
        enstrophy, umax = get_flowfeatures(
            phenotypes[i],
            domain["bitmap_resolution"],
            "data",
            domain["max_t"],
            domain["report_interval"],
            domain["mean_interval"],
        )
        features.append([area, perimeter, enstrophy, umax])

    features = np.asarray(features)
    for i in range(len(domain["feat_ranges"][0])):
        features[:, i] = maptorange.do(
            features[:, i], domain["feat_ranges"][0][i], domain["feat_ranges"][1][i]
        )
...
def get_flowfeatures(
    ...
    mask_np[
        relative_position_x : relative_position_x + building.shape[0],
        relative_position_y : relative_position_y + building.shape[1],
    ] = building
    ...
    enstrophy_reporter = lt.ObservableReporter(
        enstrophy_observable, interval=report_interval, out=f1
    )
    simulation.reporters.append(enstrophy_reporter)

    f2 = open(datadir + "/umax.csv", "a")
    maxU_reporter = lt.ObservableReporter(
        maxU_observable, interval=report_interval, out=f2
    )
    ...
    return np.mean(data_enstrophy[-mean_interval:, 2]), np.mean(
        data_umax[-mean_interval:, 2]
    )
...
def rasterize_to_disk(polygon, bitmap_resolution):
    polygon = shapely.affinity.scale(
        polygon,
        xfact=bitmap_resolution,
        yfact=bitmap_resolution,
        zfact=1.0,
        origin="center",
    )
    ...
    polygon = shapely.affinity.translate(
        polygon,
        xoff=-centroid_x[0] + bitmap_resolution / 2,
        yoff=-centroid_y[0] + bitmap_resolution / 2,
        zoff=0.0,
    )
    img = rasterio.features.rasterize(
        [polygon], out_shape=(bitmap_resolution, bitmap_resolution)
    )
    img *= 255
    with rasterio.Env():
        with rasterio.open(
            ...
            dst.write(img, 1)
            dst.write_colormap(1, {0: (0, 0, 0), 255: (255, 255, 255)})
