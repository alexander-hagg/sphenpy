from qd.mapelites.archive import MapElitesArchive
import statistics


def evolve(init, config, domain, ff):
    # Initialization
    archive = MapElitesArchive(domain, config)
    ...
    for iGen in range(config["num_gens"]):
        ...
        if iGen % 100 == 0:
            ...
            if iGen > 99:
                print(
                    "Avg. improvement in last 100 gens: "
                    + str(statistics.mean(improvement[-99:]))
                    + "%"
                )
