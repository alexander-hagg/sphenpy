...
def plot(archive, domain):
    ...
    f = interp2d(archive['features'][:,0],archive['features'][:,1],archive['fitness'],kind="linear")
    ...
    fig = plt.imshow(c_i, extent=[domain['feat_ranges'][0][0], domain['feat_ranges'][1][0], domain['feat_ranges'][0][1], domain['feat_ranges'][1][1]], origin="lower", interpolation='bicubic')
    # plt.clim(0,1)
